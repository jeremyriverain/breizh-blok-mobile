  ...
  group('get', () {
    test('requests are persisted in the database', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
          json.encode({'foo': request.url.path}),
          200,
          request: request,
      ...
      expect(
        jsonDecode(await httpClient.get(Uri.http('example.com', '/foo'))),
        {'foo': '/foo'},
      );

      final firstStoredRequest =
          await (database.select(database.dbRequests)
            ..where((tbl) => tbl.requestPath.equals('/foo'))).getSingle();

      expect(jsonDecode(firstStoredRequest.responseBody), {'foo': '/foo'});

      expect(
        jsonDecode(
          await httpClient.get(Uri.http('example.com', '/bar', {'foo': 'bar'})),
        ),
        {'foo': '/bar'},
      );

      final secondStoredRequest =
          await (database.select(database.dbRequests)..where(
            (tbl) => tbl.requestPath.equals('/bar?foo=bar'),
          )).getSingle();

      expect(jsonDecode(secondStoredRequest.responseBody), {'foo': '/bar'});
    });
    ...
    test(
      ...
      () async {
        ...
        expect(await httpClient.get(Uri.http('example.com', '/foo')), 'bar');
      },
    );
    ...
    test(
      ...
      () async {
        ...
        await expectLater(
          httpClient.get(Uri.http('example.com', '/bar')),
    ...
    test(
      ...
      () async {
        ...
        await expectLater(
          httpClient.get(Uri.http('example.com', '/foo')),
    ...
    test(
      ...
      () async {
        final mockClient = MockClient((request) async {
          return http.Response(
            json.encode({'bar': request.url.path}),
            200,
            request: request,
            headers: {'content-type': 'application/json'},
          );
...
even if the attempt to persist the response from the network fails''',
  ...
  test('normalizeRequestPath sorts query parameters alphabetically', () {
    final database = AppDatabase(NativeDatabase.memory());
    final httpClient = ApiClient(database: database, httpClient: http.Client());
    expect(
    ...
    expect(
      httpClient.normalizeRequestPath(
        Uri.https('foo.org', 'bar', {'cd': 'foo', 'ab': 'bar'}),
      ),
      '/bar?ab=bar&cd=foo',
    );
